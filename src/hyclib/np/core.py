...
def meshshape(*shapes, indexing="ij"):
    ...
    if indexing == "xy":
        if len(shapes) < 2:
            raise ValueError(
                "Must have at least 2 shapes if indexing == 'xy', but got"
                f" {len(shapes)=}."
            )
...
def meshndim(*ndims, indexing="ij"):
    ...
    if indexing == "xy":
        if len(ndims) < 2:
            raise ValueError(
                "Must have at least 2 ndims if indexing == 'xy', but got"
                f" {len(ndims)=}."
            )
    ...
    cumndim = 0
    for ndim in ndims:
        # we don't use Ellipsis because it doesn't allow for possible additional
        # batch dimensions
        yield (
            (None,) * cumndim
            + (slice(None),) * ndim
            + (None,) * (sumndim - cumndim - ndim)
        )
        cumndim += ndim
...
def meshgrid(*arrays, indexing="ij", axes=None, sparse=False):
    ...
    numpy. If axes is None, meshes over all dimensions. axes can be None, an int,
    or a length-P list/tuple of length-2 list/tuple of ints or None, where
    P = len(arrays). Negative axis has the same semantic meaning as slicing with a
    negative index. If sparse, new dimensions are of length one instead of the
    broadcasted length.

    If indexing == 'ij', returns arrays of shapes
    (
        *00, n0[axes[0][0]], ..., n0[axes[0][1]], n1[axes[1][0]], ...,
        n1[axes[1][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *01,
    ),
    (
        *10, n0[axes[0][0]], ..., n0[axes[0][1]], n1[axes[1][0]], ...,
        n1[axes[1][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *11,
    ),
    ...,
    (
        *P0, n0[axes[0][0]], ..., n0[axes[0][1]], n1[axes[1][0]], ...,
        n1[axes[1][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *P1,
    ),

    Otherwise, returns arrays of shapes
    (
        *00, n1[axes[1][0]], ..., n1[axes[1][1]], n0[axes[0][0]], ...,
        n0[axes[0][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *01,
    ),
    (
        *10, n1[axes[1][0]], ..., n1[axes[1][1]], n0[axes[0][0]], ...,
        n0[axes[0][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *11,
    ),
    ...,
    (
        *P0, n1[axes[1][0]], ..., n1[axes[1][1]], n0[axes[0][0]], ...,
        n0[axes[0][1]], ..., nP[axes[P][0]], ..., nP[axes[P][1]], *P1,
    ),
    ...
    if axes is None or isinstance(axes, int):
        ...
        and len(axes) == len(arrays)
        and all(
            (
                isinstance(axis, (list, tuple))
                and len(axis) == 2
                and all(d is None or isinstance(d, int) for d in axis)
            )
            for axis in axes
        )
    ):
    ...
    arrays = (
        ...
        for array, shape, pre_shape, post_shape in zip(
            arrays, shapes, pre_shapes, post_shapes
        )
    )
    if not sparse:
...
def unique_rows(
    arr, sorted=True, return_index=False, return_inverse=False, return_counts=False
):
    """
    Code modified from
    https://github.com/scikit-image/scikit-image/blob/main/skimage/util/unique.py
    ...
    if arr.ndim != 2:
        raise ValueError(
            f"unique_rows() only makes sense for 2D arrays, got {arr.ndim}"
        )
...
def intersect_rows(larr, rarr, return_indices=False, **kwargs):
    ...
    if larr.ndim != 2 or rarr.ndim != 2:
        raise ValueError(
            "intersect_rows() only makes sense for 2D arrays, "
            f"got {larr.ndim=}, {rarr.ndim=}"
        )
    if larr.shape[1] != rarr.shape[1]:
        raise ValueError(
            "larr and rarr must both have same number of columns, but"
            f" {larr.shape[1]=}, {rarr.shape[1]=}."
        )

    N_cols = larr.shape[1]
    larr, rarr = np.ascontiguousarray(larr), np.ascontiguousarray(rarr)
    larr_row_view, rarr_row_view = (
        larr.view(f"|S{larr.itemsize * N_cols}"),
        rarr.view(f"|S{rarr.itemsize * N_cols}"),
    )
...
def repeat(arr, repeats, chunks=None, validate=True):
    ...
    arr, repeats, chunks = (
        np.asanyarray(arr),
        np.asanyarray(repeats),
        np.asanyarray(chunks),
    )

    if validate:
        if arr.ndim != 1 or repeats.ndim != 1 or chunks.ndim != 1:
            raise ValueError(
                f"arr, repeats, and chunks must all be 1D, but {arr.ndim=},"
        ...
        if chunks.sum() != len(arr):
            raise ValueError(
                f"sum of chunks must be the length of arr, but {chunks.sum()=} and"
                f" {len(arr)=}."
            )
