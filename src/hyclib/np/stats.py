...
# by default, np.cov uses Bessel correction, in contrast to the rest of its stats
# functions
cov = np.cov
...
def weightedmeanerr(y, yerr, axis=None):
    ...
    y = np.sum(y / yerr**2, axis=axis) / np.sum(
        1 / yerr**2, axis=axis
    )  # eq. 4.17 in reference
...
def nanweightedmeanerr(y, yerr, axis=None):
    """
    Weighted mean and error of weighted mean with np.nan values (either in mean or in
    error) ignored in the calculations.
    ...
    y = np.nansum(y / yerr**2, axis=axis) / np.nansum(
        1 / yerr**2, axis=axis
    )  # eq. 4.17 in reference
