...
def flatten_dict(d, depth=-1):
    ...
    depth=0 returns the dictionary unchanged, depth=-1 returns a fully flattened dict
    depth=n means the first n layers of the dictionary is flattened, so a depth k dict
    becomes a depth k-1 dict.
    ...
    for k, v in d.items():
        ...
        if (
            depth == 0 or not isinstance(v, dict) or v == {}
        ):  # if depth == 0 or v is leaf
            flattened_dict[k] = v
        else:
...
def deep_iter(iterable, depth=-1, types=None, exclude=None):
    ...
    if not all(issubclass(t, collections.abc.Iterable) for t in types + exclude):
        raise TypeError(
            f"types and exclude must be subclasses of Iterable, but got {types=},"
            f" {exclude=}."
        )
...
def _deep_iter(iterable, depth, types, exclude, memo):
    if (
        id(iterable) in memo
    ):  # prevents infinite recursion when encountering self-referential objects
        yield iterable
    else:
...
def dict_update(d, new_d):
    for k, v in dict_iter(
        new_d, delimiter=None
    ):  # might get very slow if new_d is very large, but I can't think of a better way
