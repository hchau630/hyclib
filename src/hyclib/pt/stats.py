...
def _bincount(indices, weights=None, minlength=0):
    if weights is None or (
        not weights.requires_grad and indices.shape == weights.shape
    ):
        ...
        # the pytorch implementation of bincount is much faster when weights is None
        # for very big inputs since it does not need to create the weights tensor.
        return torch.bincount(indices, weights=weights, minlength=minlength)

    if indices.is_floating_point():
        raise TypeError(
            f"indices must be a tensor of integer dtype, but {indices.dtype=}."
        )

    if indices.ndim != 1 or weights.ndim < 1:
        raise ValueError(
            "indices must be 1D and weights must be at least 1D, but {indices.ndim=}"
            f" and {weights.ndim=}."
        )

    if len(indices) != weights.shape[-1]:
        raise ValueError(
            "Length of indices must be same as last dimension of weights, but"
            f" {len(indices)=} and {weights.shape[-1]=}."
        )

    if indices.device != weights.device:
        raise ValueError(
            "indices and weights must be on the same device, but {indices.device=} and"
            f" {weights.device=}."
        )
    ...
    shape = (
        *weights.shape[:-1],
        max(indices.max().item() + 1, minlength) if len(indices) > 0 else minlength,
    )
...
def bincount(x, weights=None, minlength=0, nan_policy="propagate"):
    """
    Similar to torch.bincount, but supports auto-differentiation and allows batched
    weights. Always performs bincount on the last dimension, with the leading
    dimensions interpreted as batch dimensions.
...
def _bin_edges(sample, bins=None, range=None):
    ...
    edges_dtype = sample.dtype if torch.is_floating_point(sample) else torch.float

    # Create edge arrays
    for i in builtins.range(Ndim):
        bins_i = torch.as_tensor(bins[i])
        if bins_i.ndim == 0:  # scalar, interpret as number of bins
            edges[i] = torch.linspace(
                smin[i], smax[i], bins_i.item() + 1, dtype=edges_dtype
            ).to(device)  # compute on cpu first since MPS seems to have precision issue
        else:
...
def _bin_numbers(sample, edges, dedges):
    ...
    sampBin = [
        torch.bucketize(
            sample[:, i], edges[i], right=True
        )  # `right` argument has opposite meaning in torch.bucketize vs np.digitize
        for i in range(Ndim)
    ]
        ...
        on_edge = (
            (
                ...
                & (
                    ...
                    == torch.round(edges[i][-1], decimals=decimal)
                )
            )
            .nonzero()
            .squeeze()
