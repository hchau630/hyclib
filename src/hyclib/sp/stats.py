...
def _calc_binned_statistic(
    Vdim, bin_numbers, result, values, stat_func, values_err=None
):
    ...
    for vv in builtins.range(Vdim):
        ...
        if values_err is not None:
            err_bin_map = _create_binned_data(
                bin_numbers, unique_bin_numbers, values_err, vv
            )
        for i in unique_bin_numbers:
            stat = (
                stat_func(np.array(bin_map[i]))
                if values_err is None
                else stat_func(np.array(bin_map[i]), np.array(err_bin_map[i]))
            )
...
def binned_statistic_dd(
    ...
    if values_err is not None:
        ...
        try:
            ...
            raise AttributeError(
                "shape of values must match shape of values_err, but got"
                f" {values.shape=} and {values_err.shape=}"
            ) from err
    ...
    if statistic != "count" and Vlen != Dlen:
        raise AttributeError(
            ...
            f"length of each `sample` dimension, but got {sample.shape=} and"
            f" {values.shape=}"
        )
...
def binned_mean_dd(x, y, yerr=None, weighted=False, nanstats=True, **kwargs):
    ...
    if yerr is not None:
        statistics = {
            "y": lambda y, yerr: getattr(npstats, f"{nan}{weighted}meanerr")(y, yerr)[
                0
            ],
            "yerr": lambda y, yerr: getattr(npstats, f"{nan}{weighted}meanerr")(
                y, yerr
            )[1],
        }
    else:
    ...
    out = binned_statistic_dd(
        x, y, values_err=yerr, statistic=statistics["y"], **kwargs
    )
...
def binned_mean(x, y, yerr=None, bins=10, range=None, **kwargs):
    ...
    result = binned_mean_dd([x], y, yerr=yerr, bins=bins, range=range, **kwargs)
