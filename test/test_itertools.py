...
def test_isconst(iterable, expected):
    if iterable == "bad_iterable":
        iterable = bad_iterable()
...
def test_deep_iter(iterable, depth, types, exclude, expected):
    for x, y in zip(
        lib.itertools.deep_iter(iterable, depth=depth, types=types, exclude=exclude),
        expected,
    ):
        assert x == y
...
def self_referential():
    ...
    out.append({"iterable": "abcd", "depth": -1, "expected": ["a", "b", "c", "d"]})
    ...
    out.append(
        {"iterable": iterable, "depth": -1, "expected": [1, 2, 3, 4, 5, 6, iterable]}
    )
...
def test_deep_iter_self_referential(idx, self_referential):
    ...
    for x, y in zip(
        lib.itertools.deep_iter(d["iterable"], depth=d["depth"]), d["expected"]
    ):
        assert x == y
...
@pytest.fixture
def d():
    return {"a": "hi", "b": {"c": 1, "d": 1.0, "e": {}}, "f": "hii"}
...
def test_dict_zip(d1, d2, mode, fillvalue, expected):
    if mode != "union":
        ...
        d = {
            k: (v1, v2)
            for k, v1, v2 in lib.itertools.dict_zip(
                d1, d2, mode=mode, fillvalue=fillvalue
            )
        }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"a": 1, "c": 5}, {"b": 3, "a": 4}, "strict", KeyError),
        (
            {"a": 1, "b": 2},
            {"c": 5, "b": 3, "a": 4},
            "strict",
