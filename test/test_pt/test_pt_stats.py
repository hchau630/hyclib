...
def test_bin_dd(D, bins, nan_policy, expectation):
    ...
    with expectation as exc_info:
        tbinnumbers, tcenters, tedges = lib.pt.stats.bin_dd(
            t, bins=bins, nan_policy=nan_policy
        )
    ...
    if exc_info is None:
        ...
        for i in range(D):
            torch.testing.assert_close(
                torch.from_numpy(centers[i]), tcenters[i], equal_nan=True
            )
...
def test_bin(bins, nan_policy, expectation):
    ...
    t = torch.tensor(arr)
    with expectation as exc_info:
        tbinnumbers, tcenters, tedges = lib.pt.stats.bin(
            t, bins=bins, nan_policy=nan_policy
        )
...
def test_bin_precision():
    """
    Previous there was a precision issue with torch.linspace(..., device='mps') that
...
def test_bincount(w, requires_grad, minlength):
    w = torch.tensor(w, requires_grad=requires_grad)
    ...
    if w.ndim == 2:
        ...
        for wi in w:
            arr.append(
                np.bincount(a.numpy(), weights=wi.detach().numpy(), minlength=minlength)
            )
        arr = np.stack(arr)
    elif w.ndim == 1:
...
@pytest.mark.parametrize("w, requires_grad", [([], True), ([[], []], False)])
@pytest.mark.parametrize("minlength", [0, 10])
def test_bincount_zero_length(w, requires_grad, minlength):
    """
    Check that the bug where lib.pt.bincount raises error on inputs with length 0 is
    fixed.
    """
    w = torch.tensor(w, requires_grad=requires_grad)
